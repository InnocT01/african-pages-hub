...
import { origins, genres, contentTypes } from "@/data/mockBooks";
...
const FilterBar = ({ activeOrigin, activeGenre, activeType, onOriginChange, onGenreChange, onTypeChange }: FilterBarProps) => {
  ...
  const PillGroup = ({ label, items, active, onChange }: { label: string; items: string[]; active: string; onChange: (v: string) => void }) => (
